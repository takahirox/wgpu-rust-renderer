...
impl WGPUAttributes {
	...
	pub fn get(&self, attribute: &Attribute) -> Option<&wgpu::Buffer> {
	...
	pub fn update(&mut self, device: &wgpu::Device, attribute: &Attribute) {
		if !self.attributes.contains_key(&attribute.get_id()) {
			self.attributes.insert(attribute.get_id(), WGPUAttributes::create_buffer(device, attribute.borrow_data()));
		}
	}
